 ...
 *     every element carries a one-click pointer to the underlying source
 ...
 *     "when".  Pulling elements from a topic map onto Marvex's Timeline
...
export const COURSE = {
  ...
  minutesTotal: 55,
  // Primary-Ed simplified track — same six lessons, framed for the
  // realities of teaching under-11s.  Stored as an alternate "lens"
  // that swaps lesson summaries, audience text and time estimates
  // when the visitor picks Primary Ed at the top of the overview.
  primaryEd: {
    audience: "Primary teachers · KS1 / KS2 / Early Years · TAs",
    minutesTotal: 30,
    description:
      "A 6-lesson primary-school adaptation of the Marvex teaching course. Same methodology, simplified maps (max 3 levels deep), age-appropriate examples, and a simplified four-branch Human Rights map for KS2 PSHE.",
    outcome:
      "By the end you'll have a 4-branch topic-overview map suitable for KS1 or KS2, every resource your class needs attached in one click, a week-by-week timeline you can share with parents, and a simplified Human Rights map for citizenship lessons.",
  },
...
export const LESSONS = [
  // ─── Lesson 1 ──────────────────────────────────────────────────────────
  {
    ...
    order: 1,
    primaryEd: {
      summary: "Why mind-maps suit children's working-memory limits better than linear notes — and the right number of branches for KS1 vs KS2.",
      tip: "Under-11s have less working memory headroom than older students, which is why the spatial layout of a mind-map helps even more. Keep your first maps to **3 levels deep, max 4 first-level branches** until your class is comfortable with the format.",
      minutesRead: 4,
    },
  ...
  // ─── Lesson 2 ──────────────────────────────────────────────────────────
  {
    ...
    order: 2,
    primaryEd: {
      summary: "A simpler blueprint: 3-4 branches, big readable labels, a question children can answer in their own words, and colour for clarity not decoration.",
      tip: "For KS1/KS2, drop the 7-15-60 rule down to **3-5 first-level branches, 10-20 total elements**. Use a question phrased the way your class would say it (e.g. 'How does a plant make its food?'). Show the map on the whiteboard and build it WITH the class — don't hand them a finished one.",
      minutesRead: 5,
    },
    visualAfter: { kind: "map", id: "MAP_TOPIC_OVERVIEW", caption: "Lesson 2 example — a topic-overview map with a question at the centre and three colour-coded categories. Five main branches, ten supporting elements." },
    ...
    faq: [
      ...
      },
      {
        ...
        a: "Marvex ships with a 'Topic overview' template for teachers — open Studio → New map → Templates → 'Topic overview (teaching)'. The template is pre-wired with the central-question slot, four colour groups, and example anchor elements.",
      },
    ],
  },
  ...
  // ─── Lesson 3 (★ Pillar 1) ─────────────────────────────────────────────
  {
    ...
    order: 3,
    primaryEd: {
      summary: "Attach the picture book PDF, the BBC clip, the visual flashcard — one click to the resource means primary-age children actually engage with it.",
      tip: "Friction kills resource-use even more in primary settings. Attach **CBBC clips, picture-book PDFs, simple infographics, audio stories** — never PDFs longer than 4 pages. Most-used pattern: a video on each major branch ('Watch this 2-minute clip about...').",
      minutesRead: 6,
    },
    visualAfter: { kind: "map", id: "MAP_ONE_CLICK_RESOURCES", caption: "Lesson 3 example — same topic map as Lesson 2, now with one-click resource badges. Cyan ↗ = PDF / link. Purple ▶ = video. Every element carries the underlying source." },
    ...
    faq: [
      {
        ...
        a: "Marvex stores PDFs in browser local storage when you drop them on a element — typical map with 20 PDFs is 15-40 MB. Browser local storage caps at ~50 MB per origin, so for very dense resource maps the desktop app (Marvex Studio for Mac/Win/Linux) handles up to 200 MB per map. Sharing the map via the share-URL streams the resources on demand, so map size doesn't slow down student access.",
      },
      {
  ...
  // ─── Lesson 4 (★ Pillar 2) ─────────────────────────────────────────────
  {
    ...
    order: 4,
    primaryEd: {
      summary: "Turn your topic map into a weekly plan you can print, share with parents, or pin on the classroom wall.",
      tip: "For primary, a half-term timeline (6 weeks) usually maps to **one branch per week**. Share the timeline as a printable PDF with parents — it's the single most-appreciated communication you can send home. Children love seeing the timeline updated each week as a 'where we've got to'.",
      minutesRead: 5,
    },
    visualAfter: { kind: "map-and-timeline", mapId: "MAP_TOPIC_OVERVIEW", timelineId: "TIMELINE_DELIVERY_PLAN", caption: "Lesson 4 — the same content rendered TWICE. Above: the topic-overview map (the 'what'). Below: the same elements pulled onto Timeline Studio as a six-week delivery plan (the 'when')." },
  ...
  // ─── Lesson 5 ──────────────────────────────────────────────────────────
  {
    ...
    order: 5,
    primaryEd: {
      summary: "Use the map as a 'fill the gaps' activity, a sticker-add exercise, or a colour-in revision game — assessment that doesn't feel like a test.",
      tip: "Skip the 'extend with three cases' assessment from the main lesson — too abstract for primary. Instead: hide labels and ask the class to 'fill in what they remember', or print the structure and have children **draw a picture in each branch** to show what they've understood. Both are assessment AND revision in one.",
      minutesRead: 4,
    },
    visualAfter: { kind: "map", id: "MAP_BLIND_RECALL", caption: "Lesson 5 example — the same Lesson 2 topic map with labels masked. Students see only the structure (branch positions and colour groups) and must recall each element's content from spatial cues alone. Studio's 'Mask labels' mode (View menu) does this in one click." },
  ...
  // ─── Lesson 6 (★ Worked example) ───────────────────────────────────────
  {
    ...
    order: 6,
    primaryEd: {
      summary: "A simplified 4-branch Human Rights map for KS2 citizenship — Safety, Fairness, Freedoms, Privacy & Family — with age-appropriate examples.",
      tip: "Use the **simplified 4-branch version** described in this lesson's FAQ rather than the full 11-branch map. Each branch carries one age-appropriate example: 'Safety' = 'You can't be hurt by anyone'; 'Fairness' = 'Everyone is treated the same'; 'Freedoms' = 'You can say what you think (kindly)'; 'Privacy & Family' = 'Your home and family are protected'. Pair with the picture book *We Are All Born Free* (Amnesty International) for KS1-2.",
      minutesRead: 6,
    },
    ...
    minutesRead: 10,
    tldr:
      "You've learned the method (lessons 1-5). This lesson applies it end-to-end on a real UK curriculum topic: the Human Rights Act 1998. Central question: 'Whose human rights does the HRA 1998 actually protect?'. Eleven main branches (one per right), one element per Article with a one-click resource attached (gov.uk, Liberty, BBC Bitesize, landmark-case PDF). Map → Timeline gives you a six-week delivery plan across Citizenship / Law / Politics. Assessment: students extend the map with one post-2020 case per right.",
    sections: [
      {
      ...
      },
      {
        ...
        paragraphs: [
          ...
          "**Article 2 — Right to life (absolute).** Sub-elements: 'state must investigate deaths in custody', 'right not to be killed unlawfully by the state', 'protection extends to known threats to life'. Landmark case: *Osman v UK (1998)* — duty to act on credible threats.",
          "**Article 3 — Freedom from torture, inhuman or degrading treatment (absolute).** Sub-elements: 'no derogation, even in wartime', 'covers inhuman or degrading treatment, not only torture', 'applies in care, detention and immigration settings'. Landmark case: *Ireland v UK (1978)* — the 'five techniques'.",
          "**Article 5 — Right to liberty and security (qualified).** Sub-elements: 'arrest only on reasonable suspicion', 'right to be told why you're arrested', 'right to a prompt hearing', 'compensation for unlawful detention'. Landmark case: *A & Others v Home Secretary (2004)* — indefinite detention of foreign terror suspects ruled unlawful.",
          "**Article 6 — Right to a fair trial (qualified).** Sub-elements: 'public hearing', 'independent court', 'presumption of innocence', 'right to legal representation', 'reasonable time'. Landmark case: *Saunders v UK (1996)* — self-incrimination.",
          "**Article 8 — Respect for private and family life (qualified).** Sub-elements: 'privacy', 'home', 'correspondence', 'family life'. Most-litigated Article in the UK. Landmark case: *Campbell v MGN (2004)* — celebrity privacy and the press.",
          "**Article 9 — Freedom of thought, belief and religion (qualified for manifestation, absolute for the belief itself).** Sub-elements: 'right to hold a belief = absolute', 'right to manifest a belief = qualified', 'no compulsion to adopt a religion'. Landmark case: *R (Williamson) v SS for Education (2005)* — corporal punishment in religious schools.",
          "**Article 10 — Freedom of expression (qualified).** Sub-elements: 'extends to offensive ideas', 'press freedom', 'restrictions must be proportionate', 'higher protection for political speech'. Landmark case: *Handyside v UK (1976)* — 'expression covers ideas that offend, shock or disturb'.",
          "**Article 11 — Freedom of assembly and association (qualified).** Sub-elements: 'peaceful protest', 'right to join a union', 'right to NOT join'. Landmark case: *DPP v Ziegler (2021)* — proportionality test for obstruction-of-highway protests.",
          "**Article 12 — Right to marry (qualified).** Sub-elements: 'right to marry someone of the opposite or same sex', 'state may regulate the formalities', 'right to found a family'. Landmark case: *Goodwin v UK (2002)* — recognition of acquired gender.",
          "**Article 14 — Protection from discrimination (modifier, not standalone).** Sub-elements: 'only operates in conjunction with another Article', 'covers sex, race, religion, language, political opinion, national or social origin, association with a minority, property, birth, or other status'. Landmark case: *Ghaidan v Godin-Mendoza (2004)* — same-sex partner tenancy rights.",
          "**Protocol 1, Article 1 — Protection of property (qualified).** Sub-elements: 'peaceful enjoyment', 'state can interfere only in the public interest', 'compensation usually required'. Landmark case: *James v UK (1986)* — leasehold reform.",
      ...
      },
      {
        ...
        paragraphs: [
          "Every Article-level element should carry at least one resource. Here's a tested set, all free to attach:",
          "**The Act itself** — link the central question element to the [Human Rights Act 1998 on legislation.gov.uk](https://www.legislation.gov.uk/ukpga/1998/42/contents). Authoritative, free, no paywall.",
          "**Plain-English explainer** — Liberty's [Human Rights Explained guide](https://www.libertyhumanrights.org.uk/issue/human-rights-act/) is the best plain-English overview for GCSE. Attach a PDF copy to the central element.",
          ...
          "**Landmark cases** — for each Article, attach the case PDF (judgment) or a 2-3 page case summary. The Liberty 'Library' and the European Court of Human Rights (HUDOC) database both host free, redistributable summaries. Drop the PDF on the Article element and students click once to read.",
          "**Video walk-throughs** — search for 'Article 8 ECHR' (or similar) on YouTube; the Liberty channel and the LawTeacher channel both have 4-6 minute explainers per Article. Paste the URL on the element and Marvex's inline player handles the rest — students watch without leaving the map.",
          "**Equality and Human Rights Commission (EHRC)** — the EHRC has free, classroom-ready briefings on every Article. Their [resources hub](https://www.equalityhumanrights.com/en/human-rights/human-rights-act) is the right link for the 'official body' element off the central question.",
        ],
      },
      {
      ...
      },
      {
        ...
        paragraphs: [
          "Following the technique from Lesson 5, ask each student to **add one post-2020 case to any three Articles** of their choice. They must (a) name the case, (b) summarise the rights issue in one sentence, (c) attach a source (Liberty article, BBC News piece, judgment PDF) to the new element.",
          ...
          "Mark by diffing each student's map against your original. Marvex's 'Compare with original' button highlights the three new elements; you check the source, the summary, and the categorisation. ~5 minutes per student.",
      ...
      },
      {
        ...
        paragraphs: [
          "For A-level Politics, extend the map with a sub-branch on **the Bill of Rights debate** — the Conservative proposals (2022-2024), the Labour position, and the broader 'should we leave the ECHR?' question. Each becomes its own element with a resource attached.",
          ...
          "For GCSE Citizenship, link to the **child-specific rights map** (United Nations Convention on the Rights of the Child) — the UNCRC sits alongside the HRA and is the more commonly-tested instrument for under-18 protection. A separate map keeps each topic clean while the linking element connects them.",
        ],
      },
    ],
    next: null,
    faq: [
      {
        ...
        a: "Yes — Marvex Studio ships with a 'UK Human Rights Act (teacher template)' under New map → Templates. It contains the eleven-branch structure described above with placeholder resource elements, ready for you to attach your preferred PDFs and videos. Edit one element, share the whole map with your students.",
      },
      {
      ...
      },
      {
        ...
        a: "Mark each landmark-case element with the case year in its label, e.g. 'Osman v UK (1998)'. Each summer, scan the most recent term's Supreme Court and ECHR judgments via the [BAILII free-text search](https://www.bailii.org/) and add any landmark rulings as new elements off the relevant Article. Most years there are 2-4 substantial additions to make. Takes ~30 minutes once a year and keeps the map fresh.",
      },
      {
